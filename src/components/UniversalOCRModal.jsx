...
export default function UniversalOCRModal({
  ...
  const runTesseractOCR = async () => {
    ...
    try {
      ...
      if (gridResults.length > 0) setActiveTab('results');
    ...
    try {
      ...
      for (let r = 0; r < numRows; r++) {
         for (let c = 0; c < numCols; c++) {
            ...
            // Paddle works better with raw/grayscale features, so we DISABLE binarization here.
            ...
            const text = results.map(box => box.text).join(' ').trim();
       ...
       for (let c = 0; c < numCols; c++) {
          ...
          if (useTesseract) {
             ...
             for (let r = 0; r < numRows; r++) {
                ...
                // Disable binarization for Paddle in Hybrid mode too.
                const cellUrl = getCellImage(imageRef.current, cropParams, 0, 0, false);
