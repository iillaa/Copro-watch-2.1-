...
* **DEFAULT STATUS:** Set `status` to `'pending'` (or `null`), **NOT** `'apte'`.
    * *Logic:* A new agent is "Neutre" until their first Commission.
...
* **Columns to HIDE:**
    ...
    * *(Note: These details remain in the database, just hidden from the list view).*

## 4. 📝 WeaponExamForm.jsx (The Commission Logic)
This is the core logic change.

### A. Form Fields
1.  **Date de Commission:** (Date Input). Default to Today. *Basis for calculations.*
2.  **Type (was Motif):** Select Dropdown.
    * *Options:* "Affection Somatique", "Affection Psychiatrique", "Affection Psychologique", "Par Précaution".
3.  ❌ **Remove:** `visual_acuity` (OD/OG).
4.  ✅ **Add:** `medical_aptitude` (Radio/Select: "Apte" or "Inapte"). *Label: "Mon Avis (Médecin)".*
5.  **Avis Psychologue:** Keep dropdown.
6.  **Avis Chef de Service:** Keep dropdown. *Label: "Avis Chef de Service (Responsable Direct)".*

### B. "Decision Finale" Logic
* **If Decision == 'Apte':**
    * `next_review_date` = `null` (Permanent).
    * *Note:* "Apte" is valid indefinitely until a new incident occurs.
